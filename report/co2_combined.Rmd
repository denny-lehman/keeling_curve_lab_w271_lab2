...
```{r}
# code directly from link
co2_present <- read.csv("https://gml.noaa.gov/webdata/ccgg/trends/co2/co2_weekly_mlo.csv",skip=51)
co2_present
```
...
co2_present_yearlymonthly <- co2_present %>%
...
# Print the RMSE
print(paste("Root Mean Squared Error (RMSE):", rmse_monthly_yearly))
...
# Assuming the "yearweek" column is in date format
# Convert it to a year-month format to compare easily
co2_present_reduced_weekly_group <- co2_present_reduced_weekly_group %>%
  mutate(yearmonth = format(yearweek, "%Y-%m"))

# Filter the data to get the test dataset (last 2 years)
test_data_non_season <- co2_present_reduced_weekly_group %>%
  filter(yearmonth >= "2022-01")%>%
  select(yearweek, mean_average)
