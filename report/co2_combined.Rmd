...
In the previous section, we performed a comprehensive evaluation of models trained on 1997 dataset with the focus on long term forecasting 1998 to 2023 and got high residual error. However, for long term forecasting, utilizing seasonally adjusted data becomes more appropriate. To accomplish this we applied the STL method on weekly data, which effectively decomposes the time series and provides us with seasonal adjusted data. This dataset was subsequently split into training (1997 to 2021) and test (2021 to 2023) sets to assess model performance.

On this seasonally adjusted weekly dataset, the ARIMA models were trained and evaluated on the test dataset, yielding an RMSE value of 0.59. Similarly we conducted a parallel analysis on the non seasonal adjusted data, resulting in an RMSE value of 2.2. Interestingly, the seasonally adjusted ARIMA model showcased slightly better performance than its non-seasonally adjusted counterpart. This make sense because ARIMA model on seasonally adjusted data can capture more accurate predictions. On other hand, if ARIMA model is used on non-seasonal data then it might struggle to capture seasonal pattern.

During the training process, we employed a grid search using fable package tool, exploring various parameter combinations (p, d, q) ranging from (0:8, 0:2, 0:8) and seasonal parameter combinations (P, D, Q) spanning from (0:12, 0:4, 0:12). The final selected ARIMA model was (0,1,1)(4,0,0)[12], indicating the implementation of differencing once for stationarity and incorporating one lag of forecast errors. This model demonstrated improved performance on the out-of-sample test dataset compared to the ARIMA model built in the previous section. This improvement in predictive capabilities can be attributed to the inclusion of more recent training data from 1997 to 2020, which allowed the model to forecast the immediate future (2021 to 2023) more accurately. Also, the ARIMA model performs well both on in-sample and out-sample, indicating that model is flexible and able to predict accurately and not overfitting.
Additionally, we compared the non-seasonally adjusted ARIMA model with a polynomial linear model trained on seasonally adjusted data. The RMSE value for the polynomial linear model was 1.7, whereas the non-seasonally adjusted ARIMA model achieved an RMSE of 2.3. The non-seasonally adjusted ARIMA model showcased inferior performance in predicting the immediate future and effectively capturing the seasonal pattern.
