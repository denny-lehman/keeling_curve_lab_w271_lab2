...
#   select(.resid) %>% 
#   ggplot() +
#   geom_histogram(aes(x=.resid))+ ggtitle("Fig.16 Histogram plot of residuals from ARIMA") +
#   theme(text = element_text(size = 8)) 
...
To ensure a fair comparison of these predictions we aggregated the current dataset to a monthly level. On seasonally adjusted data, the model performance of ARIMA model , after comparing its predictions with the current dataset, resulted in an RMSE of 0.49 On the other hand, the linear model which considered both the seasonal and trend components, had an RMSE of 6.15. It looks like that data has certain trend and pattern which are better captured by linear regression.
...
```{r split train and test data, include = FALSE}
...
test.size = 96 # fix this. what is 96?
...
```{r train lienar non seasonal model, echo = FALSE, fig.height=3 }
# train_data_non_season <- train_data_non_season%>%as_tsibble()
train_data_non_season %>% 
  model(model_trend =  TSLM(value ~ trend() + season() )) ->
  fit_linear_NON_season_weekly

```



```{r forecast linear non seasonal model}

test.size = 96 # fix please

model.forecasts_weekly_non_seasonal <-forecast(fit_linear_NON_season_weekly, h=test.size)
model.forecasts_weekly_non_seasonal  
# Merge the two data frames on the 'index' column, and handle missing values with 0
merged_df_weekly_non_seasonal <- merge(model.forecasts_weekly_non_seasonal, test_data_non_season, by = "time_index", all.x = TRUE)
