...
The following code snipped outlines the pipeline to read data from Weekly and Monthly URLs.Minor transformation have be performed in order to get the data into a time series object. 
...
p2 <- co2_present %>% index_by(year = lubridate::year(time_index)) %>%
  ...
  ggtitle("Fig.2 Annual growth rate of concentration, %")+ 
  theme(text = element_text(size = 8)) 
p2 <- co2_present %>% PACF(value) %>% autoplot()+
  ggtitle("Fig.3 ACF of CO2 concentration")+ 
  theme(text = element_text(size = 8)) 
p3 <- co2_present %>% ACF(value) %>% autoplot()+
  ggtitle("Fig.3 ACF of CO2 concentration")+ 
  theme(text = element_text(size = 8)) 
...
fc_co2_1998 <- fabletools::forecast(co2_full_trend_fit, h=(2023-1998)*12)
# Create the forecast
...
fc_co2_2022 <- fabletools::forecast(model.bic, h=(2023-1998)*12)
...
# RESULT OF SEASON - Linear RMSE is 1.7663
...
ggplot(data = p31_1, aes(x = yearmonth, y = season_adjust)) +
  geom_line() +
...
# train_data <- train_data%>%as_tsibble(index = index)
...
model.forecasts_ARIMA_monthly_yearly

```

```{r include = FALSE}
# test_data_STL_monthly_yearly <- test_data_STL %>%
#  rename(yearmonth = index)
# test_data_STL_monthly_yearly
```


```{r include = FALSE}
# ARIMA Forcast Seasonally adjusted 
...
# Merge the two data frames on the 'index' column, and handle missing values with 0
joined_dataset_monthly_yearly <- merge(model.forecasts_ARIMA_monthly_yearly, test_data_seasonally_adjusted, by = "yearmonth", all.x = TRUE)

# Select the desired columns and rename them if needed
selected_df_monthly_yearly <- joined_dataset_monthly_yearly %>%
  select(yearmonth, season_adjust.y , .mean) %>%
  rename(predicted = .mean, current_value = season_adjust.y)

# Print the result
print(selected_df_monthly_yearly)
```
...
# Plot between Current vesus Predicted ARIMA Seasonally adjusted

data.frame(time_index = selected_df_monthly_yearly$yearmonth, Actuals = selected_df_monthly_yearly$current_value, 
           Forecast = selected_df_monthly_yearly$predicted) %>%
  pivot_longer(cols=c('Actuals', 'Forecast')) -> lm_vs_actuals
...
# Plot using ggplot2
# ggplot(selected_df_yearly, aes(x = yearmonth, y = mean_average)) +
#  geom_line(aes(y = current_value), color = "blue") +
#  geom_point(color = "red", size = 3) +
#  labs(title = "Predicted vs Actual Values of CO2",
#       x = "Time index (Year-Month)",
#       y = "CO2 Levels") +
 # theme_minimal() +
...
# Forcast train_data for 

```{r include = FALSE}

# REDUNDANT

#  model name is model.bic_monthlyyearly and train dataset is train_data_seasonally_adjusted

fc_co2_2022 <- fabletools::forecast(model.bic_monthlyyearly, h=276)
...
```{r  echo = FALSE , fig.height=3}
# Linear Seasonal adjusted 

# Forecasts of Section 2 LINEAR model on Train Dataset [Jan 2022 to Dec 2023]
...
p39 <- augment(fit_linear_season_adjusted_yeary_monthly)%>%ggplot(aes(x=yearmonth)) +
...
model.forecasts__season_linear_yearly_monthly  
```

```{r include = FALSE}
test_data_seasonally_adjusted
```
...
# Merge the two data frames on the 'index' column, and handle missing values with 0
merged_df_yearly_monthly_linear <- merge(model.forecasts__season_linear_yearly_monthly, test_data_seasonally_adjusted, by = "yearmonth", all.x = TRUE)
# merged_df_yearly[is.na(merged_df)] <- 0  # Replace missing values with 0
...
# Print the result
print(selected_df_yearly_monthly_linear)
```
...
# Print the RMSE
print(paste("Root Mean Squared Error (RMSE):", rmse_seasonal_yearly_monthly))
...
## RESULT OF NON SEASON - ** The RMSE of the linear model is 1.567, but the RMSE of the ARIMA model is 0.11932, which is smaller. Therefore, # the ARIMA model is a better model**
...
# Merge the two data frames on the 'index' column, and handle missing values with 0
merged_df_weekly <- merge(model.forecasts_ARIMA_weekly, test_data_non_season, by = "yearweek", all.x = TRUE)
# merged_df_yearly[is.na(merged_df)] <- 0  # Replace missing values with 0
...
```{r include = FALSE}
train_data_non_season
```
```{r include = FALSE}
tail(train_data_non_season,35)
```

### Weekly
```{r include = FALSE}
# Year 2027 and and on Weekly Dataset

model_weekly_ARIMA_forecast <- fabletools::forecast(model.bic_weekly_ARIMA, h=276)

model_weekly_ARIMA_forecast %>%
  autoplot(train_data_non_season) +
  labs(
    title = "Forecasts of CO2 level Up To 2027 using ARIMA model",
    y = "CO2 Level", x = "Time"
  )
```

```{r include = FALSE}
tail(test_data_non_season)
```

```{r , include = FALSE }
tail(test_data_seasonally_adjusted)
```
...
# Year 2027 and and on Monthly Dataset

model_yearly_ARIMA_forecast <- fabletools::forecast(model.bic_monthlyyearly, h=1188)

model_yearly_ARIMA_forecast %>%
  autoplot(train_data_seasonally_adjusted) +
  labs(
    title = "Forecasts of monthly-aggregated CO2 levels up to 2027 using the ARIMA model",
    y = "CO2 Level", x = "Time"
  )
```
