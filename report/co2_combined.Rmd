...
# changed code : Deepika
selected_df_monthly_yearly <- model.forecasts_ARIMA_monthly_yearly %>%
  merge(test_data_seasonally_adjusted, by = "index", all.x = TRUE) %>%
  select(index, season_adjust.y, .mean) %>%
  rename(predicted = .mean, current_value = season_adjust.y) %>%
  replace_na(0)
...
* remove code for non seasonal model *

```{r train linear non seasonal model, echo = FALSE, fig.height=3 }
...
```{r forecast linear non seasonal model, echo=FALSE}

test.size = (2024-2022) * 12 * 4 # = 96
